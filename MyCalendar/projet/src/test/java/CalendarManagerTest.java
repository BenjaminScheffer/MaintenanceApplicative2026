...
class CalendarManagerTest {
    ...
    void testEventDescription() {
        Event e = new Event("REUNION","test","Moi",
                LocalDateTime.of(2000, 7, 12, 12, 0) // 12/07/2000
                , 30,"CHEZ MOI","BOB",0);
        assertEquals("Réunion : test à CHEZ MOI avec BOB",e.description());

        Event e1 = new Event("RDV_PERSONNEL","repas nouvelle année","Moi",
                LocalDateTime.of(2010, 1, 1, 12, 0), // 01/01/2010
                30,"","",0);
        assertEquals("RDV : repas nouvelle année à 2010-01-01T12:00",e1.description());

        Event e2 = new Event("PERIODIQUE","Musculation","Moi",
                LocalDateTime.of(2010, 1, 1, 14, 0) // 01/01/2010
                ,90,"","",30);
        assertEquals("Événement périodique : Musculation tous les 30 jours",e2.description());

        Event e3 = new Event("PERIODIQUE","Escalade","Moi",
                LocalDateTime.of(2008, 1, 1, 14, 0) // 01/01/2008
                ,90,"","",30);
        assertEquals("Événement périodique : Escalade tous les 30 jours",e3.description());

        Event e4 = new Event("RDV_PERSONNEL","Anniversaire LuLu","Moi",
                LocalDateTime.of(2020, 11, 7, 12, 0) // 07/11/2020
                ,30,"","",0);
        assertEquals("RDV : Anniversaire LuLu à 2020-11-07T12:00",e4.description());

        Event e5 = new Event("RDV_PERSONNEL","Anniversaire Lolo","Moi",
                LocalDateTime.of(2020, 11, 7, 12, 0) // 07/11/2020
                ,30,"","",0);
        assertEquals("RDV : Anniversaire Lolo à 2020-11-07T12:00",e5.description());

        Event e6 = new Event("RDV_PERSONNEL","Mariage Paul","Moi",
                LocalDateTime.of(2020, 11, 7, 11, 0) // 07/11/2020
                ,90,"","",0);
        assertEquals("RDV : Mariage Paul à 2020-11-07T11:00",e6.description());
    }
